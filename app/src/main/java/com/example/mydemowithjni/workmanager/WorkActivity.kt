        ...
        findViewById<Button>(R.id.workBtn).setOnClickListener {
            ...
            workManager.getWorkInfoByIdLiveData(uniqueWorkRequest.id).observe(this, { workInfo ->
                if (workInfo?.state == WorkInfo.State.SUCCEEDED) {
                    Toast.makeText(this@WorkActivity, R.string.work_completed, Toast.LENGTH_SHORT)
                        .show()
                }
            })
            ...
            // 观察工作进度
            workManager.getWorkInfoByIdLiveData(uniqueWorkRequest.id)
            ...
            // 输入合并器
            // OverwritingInputMerger 尝试将所有输入中的所有键添加到输出中.如果发生冲突,会覆盖之前设置的值
            //      由于工作请求是并行运行的，因此无法保证其运行顺序。在上面的示例中，plantName1 可以保留值 "tulip" 或 "elm"，具体取决于最后写入的是哪个值。
            //      plantName1 = "elm, plantName1 = "tulip"  => plantName1 = "tulip"
            // ArrayCreatingInputMerger 尝试合并输入,并在必要时创建数组
            //      将每个键与数组配对。如果每个键都是唯一的，您会得到一系列一元数组。如果存在任何键冲突，那么所有对应的值会分组到一个数组中。
            //      plantName1 = "elm, plantName1 = "tulip"  => plantName1 = {"elm","tulip"}
            val cache = OneTimeWorkRequestBuilder<UploadWorker>()
                .setInputMerger(ArrayCreatingInputMerger::class)
                .build()

            //线程处理
            /*
                Worker: 最简单的实现,WorkManager会在后台线程中自动运行该工作,可自己指定相应的Executor
                    WorkManager.initialize(
                                ...
                                Configuration.Builder()
                                    ...
                                    .build())
                CoroutineWorker: 为Kotlin用户建议的实现.工作在挂起函数中,可以定义运行的Dispatcher.
                    class CoroutineDownloadWorker(
                        ...
                        override suspend fun doWork(): Result {
                            withContext(Dispatchers.IO) {//指定工作线程
                ...
                RxWorker: 为RxJava用户建议的实现.可以自由选择所需的线程进行工作处理
